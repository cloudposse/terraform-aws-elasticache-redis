...
| <a name="input_final_snapshot_identifier"></a> [final_snapshot_identifier](#input\_final_snapshot_identifier) | The name of your final node group (shard) snapshot. ElastiCache creates the snapshot from the primary node in the cluster. If omitted, no final snapshot will be made. | `string` | `null` | no |
