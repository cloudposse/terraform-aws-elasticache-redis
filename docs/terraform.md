...
| <a name="input_create_security_group"></a> [create\_security\_group](#input\_create\_security\_group) | Boolean that decides whether or not to create a security group for ElastiCache. Set to `false` if you're using existing security groups that are supplied to the module via the `security_groups` variable. | `bool` | `true` | no |
...
| <a name="input_use_default_parameter_group"></a> [use\_default\_parameter\_group](#input\_use\_default\_parameter\_group) | Use the default ElastiCache parameter group resource. | `bool` | `false` | no |
